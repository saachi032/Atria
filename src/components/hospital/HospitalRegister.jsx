...
import { statesAndCities } from "../../data/locations"; // Assuming this is your location data file
import { useNavigate } from "react-router-dom";

// Comprehensive initial state for the hospital registration form
const initialFormState = {
  hospitalName: "",
  licenseNumber: "",
  hospitalType: "", // govt, pvt, trust
  address: "",
  contactNumber1: "",
  email: "",
  website: "",
...
export default function HospitalRegister() {
  ...
  async function handleSubmit(e) {
    e.preventDefault();
    setError("");
    if (!isPhoneVerified) { setError("Please verify the Point of Contact's mobile number."); return; }
    if (form.password !== form.confirmPassword) { setError("Passwords do not match."); return; }
    if (!form.agreed) { setError("Please agree to the terms."); return; }
    // password checks (min 8 chars, number, special)
    const hasNumber = /\d/;
    ...
    try {
      ...
      setLoading(false);
      if (data.success) {
        alert("Hospital registration successful! You will be redirected to the login page.");
  ...
  // --- Styling constants with new color scheme ---
  const inputStyle = "w-full p-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-blue-500 transition";
    ...
    <div className="bg-gray-50 flex items-center justify-center py-12 px-4 pt-24">
      <div className="w-full max-w-4xl bg-white p-8 md:p-12 rounded-xl shadow-lg space-y-8">
        <div className="text-center">
          <h2 className="text-3xl font-extrabold text-gray-900">Hospital Onboarding</h2>
          <p className="mt-2 text-sm text-gray-600">Register your hospital to connect with the Atria network.</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-10">
          ...
          <fieldset>
            <legend className={sectionHeaderStyle}>1. Hospital Details</legend>
            <div className="grid md:grid-cols-3 gap-6">
              <div className="md:col-span-2">
                <label htmlFor="hospitalName" className={labelStyle}>Hospital Name</label>
                <input id="hospitalName" name="hospitalName" value={form.hospitalName} onChange={handleChange} className={inputStyle} />
              </div>
              <div>
                <label htmlFor="hospitalType" className={labelStyle}>Hospital Type</label>
                <select id="hospitalType" name="hospitalType" value={form.hospitalType} onChange={handleChange} className={inputStyle}>
          ...
          <fieldset>
            <legend className={sectionHeaderStyle}>2. Contact & Location Information</legend>
            <div className="space-y-6">
              <div className="grid md:grid-cols-2 gap-6">
                <div>
                  <label htmlFor="licenseNumber" className={labelStyle}>License / Registration No.</label>
                  <input id="licenseNumber" name="licenseNumber" value={form.licenseNumber} onChange={handleChange} className={inputStyle} />
                </div>
                 <div>
                  <label htmlFor="website" className={labelStyle}>Website (Optional)</label>
                  <input id="website" name="website" type="url" value={form.website} onChange={handleChange} className={inputStyle} placeholder="https://www.example.com" />
                </div>
              </div>
              <div className="grid md:grid-cols-2 gap-6">
                 <div>
                  <label htmlFor="email" className={labelStyle}>Official Email</label>
                  <input id="email" name="email" type="email" value={form.email} onChange={handleChange} className={inputStyle} />
                </div>
                <div>
                  <label htmlFor="contactNumber1" className={labelStyle}>Contact Number (Landline/Reception)</label>
                  <input id="contactNumber1" name="contactNumber1" type="tel" value={form.contactNumber1} onChange={handleChange} className={inputStyle} />
                </div>
              </div>
              <div>
                <label htmlFor="address" className={labelStyle}>Full Address</label>
                <textarea id="address" name="address" value={form.address} onChange={handleChange} className={inputStyle} rows={2}></textarea>
              </div>
          ...
          {/* Section 3: Point of Contact (POC) Details */}
          <fieldset>
            <legend className={sectionHeaderStyle}>3. Point of Contact (POC) Details</legend>
            <div className="space-y-6">
              <div className="grid md:grid-cols-2 gap-6">
                <div><label htmlFor="pocName" className={labelStyle}>POC Full Name</label><input id="pocName" name="pocName" value={form.pocName} onChange={handleChange} className={inputStyle} /></div>
                <div><label htmlFor="pocDesignation" className={labelStyle}>POC Designation</label><input id="pocDesignation" name="pocDesignation" value={form.pocDesignation} onChange={handleChange} className={inputStyle} /></div>
              </div>
              <div className="grid md:grid-cols-2 gap-6">
                 <div>
              ...
              {isOtpSent && !isPhoneVerified && (
                <div className="p-4 bg-blue-50 rounded-md grid md:grid-cols-2 gap-6 items-center">
                  <label htmlFor="otp" className={labelStyle}>Enter OTP sent to {form.pocMobile}</label>
                  <div className="flex items-center gap-2">
                    <input id="otp" name="otp" value={otp} onChange={(e) => setOtp(e.target.value)} className={inputStyle} maxLength="4" placeholder="4-digit code" />
              ...
              <div className="grid md:grid-cols-2 gap-6">
                <div>
                  ...
                  <input id="password" name="password" type="password" value={form.password} onChange={handleChange} className={inputStyle} />
                  <p className="text-xs text-gray-500 mt-1">8+ chars, 1 number, 1 special character.</p>
                </div>
                <div><label htmlFor="confirmPassword" className={labelStyle}>Confirm Password</label><input id="confirmPassword" name="confirmPassword" type="password" value={form.confirmPassword} onChange={handleChange} className={inputStyle} /></div>
              </div>
            </div>
          </fieldset>
