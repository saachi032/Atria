...
const initialInventoryData = [
  {
    ...
    location: "Fridge A-1",
    donorId: "D-123",
    notes: "Reserved for surgery ward.",
  },
  {
    id: 2,
    type: "A-",
  ...
  },
  {
    ...
    location: "Emergency Shelf",
    donorId: "D-127",
    notes: "Urgent need expected.",
  },
  {
    id: 6,
    type: "O-",
...
const getRowStyle = (item) => {
  ...
  if (diffDays <= EXPIRY_SOON_THRESHOLD_DAYS)
    return { style: "bg-yellow-100", tooltip: `Expires in ${diffDays} day(s)` }
    ...
    <div className="flex h-screen bg-gray-50 font-sans">
      ...
      <main className="flex-1 p-8 overflow-y-auto">
        <header className="flex justify-between items-center mb-6">
          ...
          <div className="flex items-center gap-4">
            <button
              ...
              className="flex items-center gap-2 px-4 py-2 text-sm font-semibold text-gray-700 bg-white border rounded-lg shadow-sm hover:bg-gray-50"
            >
              <DownloadIcon className="w-4 h-4" /> Download Report
            </button>
            <button
              onClick={() => setAddModalOpen(true)}
              className="flex items-center gap-2 px-4 py-2 text-sm font-semibold text-white bg-red-600 rounded-lg shadow-sm hover:bg-red-700"
            >
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
          ...
          <div className="p-4 bg-white rounded-lg shadow-sm border">
            <p className="text-sm text-gray-500">Soon Expiring Units</p>
        ...
        <div className="bg-white rounded-lg shadow-sm border">
              ...
              <thead className="border-b bg-gray-50 sticky top-0">
                <tr>
                  ...
                  <th
                    ...
                    className="p-4 text-sm font-semibold text-gray-600 cursor-pointer text-right"
                  >
                    Units
                  </th>
                  <th
                ...
                {paginatedInventory.map((item) => {
                  ...
                  return (
                    <tr key={item.id} className={`border-b hover:bg-gray-100 ${style}`} title={tooltip}>
                      ...
                      <td className="p-4 text-sm text-gray-500 truncate" style={{ maxWidth: "150px" }}>
                        {item.notes}
                      </td>
                      <td className="p-4 text-center">
                        ...
                        <button onClick={() => handleDeleteClick(item)} className="p-1 text-red-600 hover:text-red-800">
                          <TrashIcon />
                        </button>
                      </td>
                    </tr>
          ...
          <div className="p-4 flex justify-between items-center text-sm">
            <p>
              Showing {paginatedInventory.length} of {filteredAndSortedInventory.length} entries
            </p>
      ...
      {isDeleteModalOpen && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white p-6 rounded-lg shadow-xl">
            ...
            <p className="my-4">
              Are you sure you want to remove stock ID {currentItem?.id} ({currentItem?.type})?
            </p>
            <div className="flex justify-end gap-4">
              <button onClick={() => setDeleteModalOpen(false)} className="px-4 py-2 bg-gray-200 rounded">
                Cancel
              </button>
