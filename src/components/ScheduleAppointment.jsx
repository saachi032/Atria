...
export default function ScheduleAppointment() {
  const [formData, setFormData] = useState({
    ...
    date: '',
    time: '',
    fullName: 'Alex Doe', // Pre-filled example
    email: 'alex.doe@email.com', // Pre-filled example
    phone: '+91 98765 43210', // Pre-filled example
  });
  ...
  const handleEligibilityCheck = () => {
    ...
    const waitPeriodDays = 56; // Standard 56 days for whole blood
    ...
    const formattedLastDate = lastDonationDate.toLocaleDateString('en-IN', {
        day: '2-digit', month: 'long', year: 'numeric'
    });
    
    // Format for the date input's `min` attribute (YYYY-MM-DD)
    ...
    if (today >= nextEligibleDate) {
        setEligibility({
            checked: true,
            isEligible: true,
            message: `You are eligible to donate. Your last donation was on ${formattedLastDate}.`,
        ...
        setEligibility({
            checked: true,
            isEligible: false,
            message: `You are not eligible yet. Your next eligible donation date is after ${formattedNextDate}.`,
            minDate: minDateForInput
        });
    }
  };
  ...
  const handleSubmit = (e) => {
    ...
    if (!eligibility.isEligible) {
        alert("Please confirm your eligibility before submitting.");
        return;
    }

    // Double-check date validity in case the `min` attribute is bypassed
    const preferredDate = new Date(formData.date);
    const eligibleDate = new Date(eligibility.minDate);

    if (preferredDate < eligibleDate) {
        alert(`Your selected date is before your eligible donation date. Please choose a later date.`);
        return;
    }
  ...
  // Helper to format date for the confirmation screen
    ...
    return (
        <div className="min-h-screen bg-gray-50 flex flex-col justify-center items-center text-center pt-20 px-4">
            <div className="bg-white p-8 md:p-12 rounded-2xl shadow-xl max-w-2xl">
                ...
                <button 
                    onClick={() => window.location.reload()} // Reload to reset the form state
                    className="mt-8 px-6 py-3 bg-red-600 text-white font-bold rounded-lg shadow-md hover:bg-red-700 transition"
                >
    ...
    <div className="min-h-screen bg-gray-50 pt-28 pb-16">
      <div className="text-center mb-10 px-4">
        ...
        <p className="text-gray-500 mt-2 max-w-2xl mx-auto">
          Your decision to donate can save a life. Please fill out the form below to book your slot.
        </p>
      </div>
        ...
        <form onSubmit={handleSubmit} className="p-4 sm:p-6 lg:p-8 max-w-4xl mx-auto">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-6">
            ...
            <div className="md:col-span-2">
                <h2 className="text-xl font-bold text-gray-700 border-b pb-2 mb-4">Your Information</h2>
            </div>
            ...
            <div className="md:col-span-2 mt-4">
                ...
                <div className="bg-gray-50 p-4 rounded-lg flex flex-col sm:flex-row items-start sm:items-end gap-4">
                    ...
                    <button type="button" onClick={handleEligibilityCheck} className="flex-shrink-0 w-full sm:w-auto px-5 py-3 bg-blue-600 text-white font-semibold rounded-lg shadow-md hover:bg-blue-700 transition">
                        Check Eligibility
                    </button>
                </div>
                 {eligibility.checked && (
                    <div className={`mt-4 p-3 rounded-md text-sm font-medium ${eligibility.isEligible ? 'bg-green-100 text-green-800' : 'bg-yellow-100 text-yellow-800'}`}>
                        {eligibility.message}
                    </div>
                )}
            </div>
            ...
            <fieldset disabled={!eligibility.isEligible} className="md:col-span-2 mt-4 contents">
                <div className="md:col-span-2">
                    <h2 className={`text-xl font-bold border-b pb-2 mb-4 ${!eligibility.isEligible ? 'text-gray-400' : 'text-gray-700'}`}>Step 2: Appointment Details</h2>
                </div>
                ...
                <div className="md:col-span-2 text-center mt-6">
                  <button
                    type="submit"
                    className="w-full md:w-auto inline-flex items-center justify-center px-12 py-4 bg-red-600 text-white font-bold rounded-xl shadow-lg hover:bg-red-700 transition-all duration-300 transform hover:-translate-y-1 disabled:bg-red-300 disabled:cursor-not-allowed disabled:transform-none"
                  >
                    Confirm Appointment
                  </button>
                </div>
